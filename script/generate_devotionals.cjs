...
for (const file of files.sort()) {
  ...
  if (matches) {
    for (let i = 0; i < matches.length; i += 2) { // Take 2 paragraphs per devotional
      ...
      const p1 = matches[i] ? matches[i].replace(/<\/?p.*?>/g, '').replace(/&quot;/g, '"').replace(/&#8217;/g, "'").replace(/<[^>]*>?/gm, '').trim() : '';
      const p2 = matches[i+1] ? matches[i+1].replace(/<\/?p.*?>/g, '').replace(/&quot;/g, '"').replace(/&#8217;/g, "'").replace(/<[^>]*>?/gm, '').trim() : '';
